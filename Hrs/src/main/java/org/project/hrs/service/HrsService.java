...
/**
 * HrsService предоставляет функциональность для обработки запросов, связанных с системой управления ресурсами (HRS).
 * Он выполняет операции по проверке месяца и расчету тарифов на основе полученных запросов.
 * В классе реализован метод createTariff(), который инициализирует базовые тарифы и их компоненты при старте приложения.
 * Метод monthCheck() выполняет проверку месяца и возвращает список тарифов, актуальных на текущий месяц.
 * Метод tariffCalculation() производит расчет тарифа на основе запроса BrtRequest, учитывая наличие компонентов тарифа
 * и историю использования ресурсов. Для каждого запроса создается объект HrsRequest с информацией о списании средств.
 */
